...
def generate():
    ...
    photo_w = 680    # 照片寬度
    photo_h =  817# 高度
    center_x = 425  # 中心點 X（往右一點）
    center_y = 817 #中心點往上一點
    ...
    for i, line in enumerate(dept_lines):
        ...
        y = int(H * 0.37) + i * int(H * 0.075)   # 系所文字高度
        draw.text((x, y), line, fill="black", font=f)

    # 個人資訊
    info_font = ImageFont.truetype(font_path, int(H * 0.06))
    draw.text((photo_x, int(H * 0.82)), f"姓名: {name}", fill="black", font=info_font)   # 姓名高度
    draw.text((photo_x, int(H * 0.9)), f"學號: {student_id}", fill="black", font=info_font)  # 學號高度
    draw.text((int(W * 0.36), int(H * 0.9)), f"性別: {gender}", fill="black", font=info_font)  # 性別高度
